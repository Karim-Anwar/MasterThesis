...
def gen_caltech_path(root_path):
    label_path = 'labels_with_ids/train'
    real_path = os.path.join(root_path, label_path)
    image_path = real_path.replace('labels_with_ids', 'images')
    images_exist = sorted(glob.glob(image_path + '/*.jpeg'))
    with open('FairMOT_ROOT/src/data/mot15.train', 'w') as f:
